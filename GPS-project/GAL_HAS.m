function [dNEVs,Error_3D,Error_H] = GAL_HAS(eph,GIM_PATH,GS_RANGE,arrQM,gw, gs_start)
addpath('modules/')
%% CONSTANTS
obsType = 103;
...
%% Load Has File
[orbit, clock, ~, e_bias]= read_has_mat(gw,gs_start);
...
for ke = 1:NoEpochs
    gs = FinalTTs(ke);
    idx1e = find(arrQM(:,1) == gs & arrQM(:,3) == obsType & arrQM(:,2) > 400 & arrQM(:,2) < 500 );
    QM1e = arrQM(idx1e,:);
    NoSats = height(QM1e);
    clc;
    fprintf('GAL_HAS : epoch:\r%d/%d\n',ke,NoEpochs);
